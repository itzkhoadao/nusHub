export default function VoteBlock({ count = 0, onUpvote, active = false }) {
  ...
  return (
    <button
      ...
      aria-pressed={active}
      className="vote-block group inline-flex shrink-0 self-start"
      onClick={handleClick}
      type="button"
    >
      <span
        className={`vote-block-surface flex h-10 items-center gap-1.5 border px-3.5 transition-all duration-200 ${
          active
