...
async function createGroupPostAttachmentUploadUrls(
  groupId: string,
  files: File[],
) {
...
export function validateGroupPostAttachments(files: File[]) {
  if (files.length > MAX_POST_ATTACHMENTS) {
    throw new Error(`You can attach up to ${MAX_POST_ATTACHMENTS} files`);
  }

  const oversizedFile = files.find(
    (file) => file.size > MAX_GROUP_POST_ATTACHMENT_SIZE_BYTES,
  );

  if (oversizedFile) {
    throw new Error(`${oversizedFile.name} is bigger than 15 MB`);
  }
}
...
export async function uploadGroupPostAttachments(
  groupId: string,
  files: File[],
) {
  validateGroupPostAttachments(files);

  if (files.length === 0) return [];

  const { uploads } = await createGroupPostAttachmentUploadUrls(
    groupId,
    files,
  );
